...
class AcceptFriendshipRequestUseCaseTest {
    ...
    @Before
    fun setup() {
        useCase = AcceptFriendshipRequestUseCase(dataStorePreferences, friendshipRepository, getErrorFromThrowableUseCase)
    }
    ...
    fun `GIVEN the happy path WHEN invoke THEN return the expected result`() {
        ...
        coEvery { friendshipRepository.acceptFriendshipRequest(user.id, friendID) } returns Unit.right()
    ...
    fun `GIVEN a Throwable WHEN invoke THEN return the expected result`() {
        ...
        coEvery { dataStorePreferences.getUser() } returns flowOf(user)
        coEvery { friendshipRepository.acceptFriendshipRequest(user.id, friendID) } returns throwable.left()
