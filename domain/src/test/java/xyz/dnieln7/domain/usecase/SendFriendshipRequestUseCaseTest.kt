...
class SendFriendshipRequestUseCaseTest {
    ...
    @Before
    fun setup() {
        useCase = SendFriendshipRequestUseCase(friendshipRepository, getErrorFromThrowableUseCase)
    }
    ...
    fun `GIVEN the happy path WHEN invoke THEN return the expected result`() {
        ...
        coEvery { friendshipRepository.sendFriendshipRequest(userID, friendID) } returns Unit.right()
    ...
    fun `GIVEN the unhappy path WHEN invoke THEN return the expected result`() {
        ...
        coEvery { friendshipRepository.sendFriendshipRequest(userID, friendID) } returns throwable.left()
