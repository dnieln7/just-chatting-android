...
class DeleteFriendshipUseCaseTest {
    ...
    @Before
    fun setup() {
        useCase = DeleteFriendshipUseCase(friendshipRepository, getErrorFromThrowableUseCase)
    }
    ...
    fun `GIVEN the happy path WHEN invoke THEN return the expected result`() {
        ...
        coEvery { friendshipRepository.deleteFriendship(userID, friendID) } returns Unit.right()
    ...
    fun `GIVEN the unhappy path WHEN invoke THEN return the expected result`() {
        ...
        coEvery { friendshipRepository.deleteFriendship(userID, friendID) } returns throwable.left()
