...
class DeleteFriendshipUseCaseTest {
    ...
    private lateinit var useCase: DeleteFriendshipUseCase

    @Before
    fun setup() {
        useCase = DeleteFriendshipUseCase(dataStorePreferences, friendshipRepository, getErrorFromThrowableUseCase)
    }
    ...
    fun `GIVEN the happy path WHEN invoke THEN return the expected result`() {
        ...
        coEvery { friendshipRepository.deleteFriendship(user.id, friendID) } returns Unit.right()
    ...
    fun `GIVEN a Throwable WHEN invoke THEN return the expected result`() {
        ...
        coEvery { dataStorePreferences.getUser() } returns flowOf(user)
        coEvery { friendshipRepository.deleteFriendship(user.id, friendID) } returns throwable.left()
